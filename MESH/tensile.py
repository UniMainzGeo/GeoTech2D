...
def main():
    ...
    arc        = add_arc(xc, yc, r, n, 0.0, np.pi) 
    add_points = arc + [(0.0, 0.0), (0.0, H), (L, H), (L, 0.0)]
    facets     = gen_facets(points, add_points)
    points     = points + add_points

    # close arc from the bottom 
    m = len(points)
    points = points + [(xc, yc)]
    ...
    regions.append((xc, yc+r/2, 2, area))
    ...
    markers[nf+1] = 2 # left facet
    markers[nf+3] = 3 # right facet
    markers[nf]   = 4 # bottom facet
    markers[nf+4] = 4
    markers[nf+5] = 4
    markers[nf+6] = 4
    markers[nf+2] = 5 # top facet
